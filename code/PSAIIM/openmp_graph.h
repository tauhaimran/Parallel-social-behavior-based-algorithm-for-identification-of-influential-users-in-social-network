...
private:
    ...
    void DFS_SCC(long int v, std::vector<std::vector<long int>>& components) {
        ...
        if (lowlink[v] == level[v]) {
            ...
            #pragma omp critical(components)
    ...
    bool verifyInfluentialUsers(const std::vector<long int>& influential_nodes, bool is_seed_selection, int expected_size) {
        ...
        if (is_seed_selection && is_valid) {
            ...
            #pragma omp parallel for reduction(&&:is_valid)
            for (size_t i = 0; i < influential_nodes.size(); ++i) {
                long int seed = influential_nodes[i];
...
public:
    ...
    std::vector<std::pair<long int, double>> getTopInfluentialNodes() {
        ...
        node_influence.reserve(MAX_NODES);
        #pragma omp parallel for
        for (long int i = 0; i < MAX_NODES; ++i) {
            #pragma omp critical(node_influence)
    ...
    long int determineMaxNodes(const std::vector<std::string>& files) {
        ...
        for (const auto& filename : files) {
            ...
            std::string line;
            long int local_max_id = 0;
            while (std::getline(file, line)) {
                if (line.empty() || line[0] == '#') continue;
                long int from, to;
                double weight;
                if (sscanf(line.c_str(), "%ld %ld %lf", &from, &to, &weight) >= 2) {
                    local_max_id = std::max(local_max_id, std::max(from, to));
                }
            }
            file.close();
            #pragma omp critical(max_id)
            max_id = std::max(max_id, local_max_id);
            #pragma omp critical(log)
            log_file << "Processed file " << filename << ", max node ID found: " << local_max_id << "\n";
        }
    ...
    void addEdge(const std::string& filename, long int from, long int to, double weight = 1.0) {
        ...
        if (filename.find("higgs-social_network") != std::string::npos) {
            #pragma omp critical(node_update)
            {
            ...
            for (size_t i = 0; i < lines.size(); ++i) {
                ...
                if (filename == "higgs-interests.txt") {
                    ...
                    if (!(iss >> node_id)) {
                        #pragma omp critical(output)
                        {
                            std::cerr << "Invalid line " << i + 1 << " in " << filename << ": " << line << "\n";
                            log_file << "Invalid line " << i + 1 << " in " << filename << ": " << line << "\n";
                        }
                        continue;
                    }
                    while (iss >> interest) {
                    ...
                    try {
                        ...
                        #pragma omp critical(output)
                        {
                            std::cerr << "Error parsing line " << i + 1 << " in " << filename << "\n";
                            log_file << "Error parsing line " << i + 1 << " in " << filename << "\n";
                        }
                    }
                }
            }
        ...
        for (size_t i = 0; i < communities.size(); ++i) {
            ...
            for (long int w : nodes[v].followers) {
                ...
                if (level[v] == level[w] && !is_cac[w_comm_id] && communities[w_comm_id].size() < 5) {
                    #pragma omp critical(communities)
                    {
    ...
    void calculateInfluencePower() {
        ...
        double ip_sum = 0.0;
        #pragma omp parallel for reduction(+:ip_sum)
        for (long int i = 0; i < MAX_NODES; ++i) {
            ip_sum += nodes[i].influence_power;
        }
        if (ip_sum > 0) {
            #pragma omp parallel for
            for (long int i = 0; i < MAX_NODES; ++i) {
    ...
    std::vector<long int> selectSeedCandidates() {
        ...
        #pragma omp critical(output)
        {
            std::cout << "Selected " << candidates.size() << " seed Theodores\n";
