...
class Graph
{
        ...
        for (size_t i = 0; i < communities.size(); ++i)
        {
            ...
            for (long int v : communities[i])
            {
                ...
                for (long int w : nodes[v].followers)
                {
                    ...
                    if (w >= MAX_NODES || w < 0 || nodes[w].community_id == -1)
                        continue;
        ...
        for (size_t i = 0; i < communities.size(); ++i)
        {
            ...
            if (!merged)
            {
                ...
                for (long int u : nodes[v].followers)
                {
                    ...
                    if (u >= MAX_NODES || u < 0 || nodes[u].community_id == -1)
                        continue;
    ...
    std::vector<long int> selectSeeds(int k)
    {
        ...
        std::sort(candidates.begin(), candidates.end(),
                  ...
                  { return nodes[a].influence_power > nodes[b].influence_power; });
